...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // Trigger layout so styles are calculated
        progressPath.getBoundingClientRect();

        // Apply transition after setup
        ...
        var offset = 50;     // px from top
        var duration = 550;  // scroll animation duration in ms

        $(window).on('scroll', function () {
            if ($(this).scrollTop() > offset) {
                $('.progress-wrap').addClass('active-progress');
                $('.rts-switcher').addClass('btt__visible');
            } else {
                $('.progress-wrap').removeClass('active-progress');
                $('.rts-switcher').removeClass('btt__visible');
            }
        });
